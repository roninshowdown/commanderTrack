...
const COMMANDERS: { name: string; colors: MtgColor[]; img: string }[] = [
...
export function generateFullMockDataSet() {
	const players: Player[] = MOCK_PLAYERS.map((p) => ({ ...p, id: uid() }));
	const decks: Deck[] = [];

	players.forEach((player, pi) => {
		const count = 1 + Math.floor(Math.random() * 3);
		for (let i = 0; i < count; i++) {
			const c = COMMANDERS[(pi * 3 + i) % COMMANDERS.length];
			decks.push({ id: uid(), playerId: player.id, commanderName: c.name, commanderImageUrl: c.img, colors: c.colors });
		}
	});
	...
	for (let g = 0; g < 12; g++) {
		...
		const pIds = shuffled.map((p) => p.id);
		const dIds = pIds.map((pid) => {
			const d = decks.filter((d) => d.playerId === pid);
			return d.length ? d[Math.floor(Math.random() * d.length)].id : '';
		});
		...
		for (let l = 0; l < 20 + Math.floor(Math.random() * 30); l++) {
			...
			const isCmd = Math.random() < 0.2;
			const val = isCmd ? -(1 + Math.floor(Math.random() * 5)) : (Math.random() > 0.5 ? 1 + Math.floor(Math.random() * 4) : -(1 + Math.floor(Math.random() * 6)));
			logEntries.push({
				id: uid(), gameId: gid, playerId: pid, playerName: pName, value: val,
				type: isCmd ? 'commander_damage' : 'life',
				sourcePlayerId: isCmd ? pIds.filter((x) => x !== pid)[0] : undefined,
				timestamp: createdAt + l * 5000
			});
		}
	}

	return { players, decks, gameRecords, logEntries };
}
